  ...
  static Future<void> createOrUpdateDay(int duration) async {
    ...
    if (duration > 0) {
      ...
      String startingDateString =
          DateFormat('yyyy-MM-dd HH:mm').format(startingDate);

      DateTime midnight = DateTime(
          currentDate.year, currentDate.month, currentDate.day, 0, 0, 0);

      String midnightString = DateFormat('yyyy-MM-dd HH:mm').format(midnight);
      ...
      if (currentDate.subtract(Duration(minutes: duration)).isBefore(DateTime(
        ...
        print(previousDayDuration);
        print(nextDayDuration);

        // Query the database for the date now
        var results = await db.query('calendar',
            where: "date = ?", whereArgs: [DateFormat("yyyy-MM-dd").format(currentDate)]);
        ...
        // Query the database for the date now
        var resultsPreviousDay = await db
            .query('calendar', where: "date = ?", whereArgs: [DateFormat("yyyy-MM-dd").format(startingDate)]);
